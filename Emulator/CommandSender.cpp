...
#define SEND_COUNT   10
	...
	unsigned int __stdcall CommandSender::SendNet(char data[])
	{
		WSADATA wsd;
		...
		rc = getaddrinfo("127.0.0.1", "1234", &hints, &result);
		if (rc != 0) {
	...
	int CommandSender::SendToServer(char data[],int byteLength, SOCKET ConnectSocket)
	{
		return send(ConnectSocket, data, byteLength, 0);
	}
